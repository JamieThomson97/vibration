<template>
  <div class="wrapper">
    ...
    <vuetify-audio style="display:none;" ref="vueAudio" v-if="URL" :file="URL"></vuetify-audio>
  </div>
</template>
  ...
  methods: {
    storeFile(e) {
      ...
      if (
        ...
        this.URL = window.URL.createObjectURL(e.target.files[0]);
    ...
    addMix() {
      var firstPromises = [];
      var mixPromises = [];
      const duration = this.$refs.vueAudio.duration;
      ...
      if (this.show.length > 1) {
        ...
        var doesShowExist = database
          .collection("shows")
          .where("name", "==", this.show)
          .get()
          .then(response => {
      ...
      Promise.all(produceruIDPromise)
        .then(responses => {
          ...
          return Promise.all(firstPromises).then(response => {
            ...
            return Promise.all(storagePromises)
              .then(storagePromisesResponse => {
                ...
                if (isShow) {
                  if (isXCreated) {
                    ...
                    mixPromises.push(
                      database.collection("shows").add(showData)
                    );
                  }
                }
                if (isEvent) {
                ...
                mixPromises.push(
                  database
                    ...
                    .doc(NmID)
                    .set({ dateUploaded: new Date(), uIDs: {} })
                );
                mixPromises.push(
                  database
                    .collection("mixes")
                ...
                console.log(mixData);
                const indexFunction = firebase
                  .functions()
                  .httpsCallable("indexMix");

                mixPromises.push(
                  indexFunction({ mixData: mixData, NmID: NmID })
                );
                producers.forEach(producer => {
              ...
              .then(mixPromisesResponses => {
                ...
                if (!isXCreated) {
                  if (isShow) {
                    const indexShowFunction = firebase
                      .functions()
                      .httpsCallable("indexShow");
                    finalPromises.push(
                      indexShowFunction({ showData: showData, eID: newID })
                    );
                    producers.forEach(producer => {
                      finalPromises.push(
                        database
                    ...
                    const indexEventFunction = firebase
                      .functions()
                      .httpsCallable("indexEvent");
                    finalPromises.push(
                      indexEventFunction({ eventData: eventData, eID: newID })
                    );

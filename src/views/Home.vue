<template>
  <div class="homeWrapper" v-if="customer.playlists.history">
      ...
      <div>
        ...
        <div class="timelineTiles homeTiles" v-if="customer.playlists.timeline.length > 0">
          <mixTile
            ...
            :key="mix.mID"
            :object="mix"
            playerTracksReference="customer.playlists.timeline"
          ></mixTile>
        </div>
        <div v-else style="margin-left:15px;">Follow some producers to see mixes in your timeline</div>
      </div>
    </div>
    <div class="outline history">
      ...
      <div
        class="historyTiles homeTiles"
        v-if="customer.playlists.history.length > 0 && !showLiked"
      >
        <mixTile
          v-for="mix in customer.playlists.history"
          :key="mix.mID"
          :object="mix"
          deletable="1"
          collection="history"
      ...
      <div class="historyTiles homeTiles" v-if="customer.playlists.liked.length > 0 && showLiked">
        <mixTile
          v-for="mix in customer.playlists.liked"
          :key="mix.mID"
          :object="mix"
          deletable="1"
          collection="history"
    ...
    <div class="outline listenLater">
      ...
      <div class="listenLaterTiles homeTiles" v-if="customer.playlists.listenLater.length > 0">
        <mixTile
          ...
          :object="mix"
          playerTracksReference="listenLater"
        ></mixTile>
      </div>
      <div
        v-else
        style="margin-left:15px;"
      >Here is a quick access playlist for easily recording mixes you want to listen to later</div>
    </div>

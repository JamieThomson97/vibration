                ...
                <v-fade-transition>
                  ...
                  >
                    ...
                    <v-icon
                      ...
                      @click="handleClickTrack(object, playerTracksReference)"
                      x-large
                    ...
                    <v-icon
                      ...
                      class="material-icons"
                      style="position:absolute; top:0;right:0;"
                      @click="deleteMix(object.mID)"
                    >clear</v-icon>
                  </div>
                </v-fade-transition>
              </v-img>
            </v-flex>
            <v-card-title :class="`${hover? 'mixTileInfoHover': 'mixTileInfo'}`" primary-title>
              <div
                class="mixTileTitle"
                @click="navigateMix(object.mID, object.title )"
              >{{object.title}}</div>
              <div
                v-for="x in object.producers"
                :key="x.uID"
                class="mixTileProducer"
              ...
              <div v-if="object.show" class="mixTileDate">
                ...
                <v-icon
                  ...
                  @click="addToPlaylist(object , playlistChoice)"
                >done</v-icon>
              </div>
              <div
                v-else-if="object.event"
                class="mixTileDate"
                @click="nagivateEvent(object.eID, object.name )"
              >
        ...
        <v-fade-transition>
          ...
          >
            ...
            >
              <v-icon
                class="tileActionsIcons"
                @click="addToPlaylist(object , ['Liked Mixes'])"
              >favorite_border</v-icon>
              <v-spacer></v-spacer>
              <v-icon
                class="tileActionsIcons"
                @click="addToPlaylist(object , ['Listen Later'])"
              >watch_later</v-icon>
              <v-spacer></v-spacer>
              <v-icon
                class="tileActionsIcons"
                @click="playlistSelector = !playlistSelector"
              >playlist_add</v-icon>
            </div>
          </v-card>
        </v-fade-transition>
